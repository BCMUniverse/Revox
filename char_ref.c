...
static void add_no_digit_error(struct InternalParser* parser, Utf8Iterator* input) {
  Error* error = add_error(parser);
  if (!error) {
    return;
  }
  utf8iterator_fill_error_at_mark(input, error);
  error->type = ERR_NUMERIC_CHAR_REF_NO_DIGITS;
}

static void add_codepoint_error(struct InternalParser* parser, Utf8Iterator* input, ErrorType type, int codepoint) {
  Error* error = add_error(parser);
  if (!error) {
    return;
  }
...
static void add_named_reference_error(struct InternalParser* parser, Utf8Iterator* input, ErrorType type, StringPiece text) {
  Error* error = add_error(parser);
  if (!error) {
    return;
  }
...
static bool consume_numeric_ref(struct InternalParser* parser, Utf8Iterator* input, int* output) {
  ...
  if (utf8iterator_current(input) != ';') {
    add_codepoint_error(parser, input, ERR_NUMERIC_CHAR_REF_WITHOUT_SEMICOLON, codepoint);
    status = false;
  } else {
    utf8iterator_next(input);
  }
  ...
  if (replacement != -1) {
    add_codepoint_error(parser, input, ERR_NUMERIC_CHAR_REF_INVALID, codepoint);
    *output = replacement;
    return false;
  }
...
static const NamedCharRef* find_named_char_ref(Utf8Iterator* input) {
  ...
  for (int i = 0; kNamedEntities[i].codepoints.first != -1; ++i) {
    ...
    if (utf8iterator_maybe_consume_match(input, current->name, current->length, true)) {
      assert(current->name != NULL);
      assert(current->length > 0);
      assert(current->codepoints.first != kNoChar);
      return current;
    }
  }
  return NULL;
...
static bool maybe_add_invalid_named_reference(struct InternalParser* parser, Utf8Iterator* input) {
  ...
  if (c == ';') {
    ...
    add_named_reference_error(parser, input, ERR_NAMED_CHAR_REF_INVALID, bad_ref);
    return false;
  }
  return true;
}

static bool consume_named_ref(struct InternalParser* parser, Utf8Iterator* input, bool is_in_attribute, OneOrTwoCodepoints* output) {
  assert(output->first == kNoChar);
  ...
  if (char_ref) {
    ...
    if (last_char == ';') {
      *output = char_ref->codepoints;
      ...
      add_named_reference_error(parser, input, ERR_NAMED_CHAR_REF_WITHOUT_SEMICOLON, bad_ref);
      *output = char_ref->codepoints;
      assert(output->first != kNoChar);
      return false;
    }
...
bool consume_char_ref(struct InternalParser* parser, struct InternalUtf8Iterator* input, int additional_allowed_char, bool is_in_attribute, OneOrTwoCodepoints* output) {
  ...
  utf8iterator_next(input);
  int c = utf8iterator_current(input);
  output->first = kNoChar;
  output->second = kNoChar;
